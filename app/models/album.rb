...
class Album < ApplicationRecord
  ...
  def reload_from_flickr!
    ...
    Photo.delete_all("album_id = #{id}")
    reload
    flickr_photos.to_hash["photo"].each do |photo_hash|
      photos.find_or_create_by_flickr_id(photo_hash["id"]) do |photo|
  ...
  def create_post
    content = %(<div class="collection">
                <p>
                <a href='#{Rails.application.routes.url_helpers.seo_album_path(collection, self)}' target="" rel=""><img alt="" src='#{main_photo.try(:url_small).presence || iconsmall}' class="center-block img-thumbnail"></a>
